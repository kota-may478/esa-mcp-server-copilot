...
### Publish this project on GitHub (step by step)

1. Create a new empty repository on GitHub (do not add `.env` there).
...
```bash
git add .
git commit -m "Initial release: esa MCP server for GitHub Copilot"
```
...
7. On GitHub, set repository visibility (public/private) based on your policy.
8. Add repository description and topics (example: `mcp`, `github-copilot`, `esa`, `typescript`).
9. Optionally create a tag/release:
...
## GitHub で公開する手順（Step by Step）

1. GitHub で空の新規リポジトリを作成します（README 自動生成は不要）。
2. このプロジェクトのディレクトリでターミナルを開きます。
3. まだ git 初期化していない場合は次を実行します。
...
7. GitHub 上で公開範囲（Public/Private）を設定します。
8. リポジトリ説明とトピック（例: `mcp`, `github-copilot`, `esa`, `typescript`）を設定します。
9. 必要ならバージョンタグを作成します。
